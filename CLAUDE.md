...
- `knowledge/` — the candidate knowledge store, the source of truth for every claim on a CV.  
  ...
  - `generated/` — written by AI: the structured synthesis of `raw/` plus whatever the user says in conversation. Regenerable, so safe to rewrite. All organisation of the candidate's material lives here, never in `raw/`.
